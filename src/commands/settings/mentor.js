  ...
  return Promise.resolve().then(async () => {
    try {
      if (message.author.permLevel < 3) {
        if (client.cds.has(`Mentor-${message.channel.id}`)) return message.channel.send({ embed: { color: 'RED', description: `That command is on a cooldown for another ${moment.preciseDiff(moment(client.cds.get(`Mentor-${message.channel.id}`)).add(10, 'm'), moment())}` } });
