  ...
  permLevels: [
    ...
    },
    {
      ...
      check: (message, member) => {
        try {
          const mentorRoles = (message || member).settings.mentorRoles.filter(r => (message || member).guild.roles.resolve(r.roleID));
          if (mentorRoles) {
            let mentorRoleIDs = mentorRoles.map(r => r.roleID)
            if (mentorRoleIDs.some(id => (message ? message.member : member).roles.cache.has(id))) return true;
          }
        } catch (e) {
          return false;
        }
